...
def _predict_vertices(param, roi_bbox, dense, transform=False):
    vertex = reconstruct_vertex(param, dense=dense,transform=transform)
    sx, sy, ex, ey = roi_bbox
    # src_pts = np.array([[sx, sy], [sx, ey], [ex, sy]])
    # DST_PTS = np.array([[0,0], [0,120 - 1], [120 - 1, 0]])
    # tform = estimate_transform('similarity', src_pts, DST_PTS)

    scale_x = (ex - sx) / (120 - 1)
    scale_y = (ey - sy) / (120 - 1)
    my_vertices = vertex.copy() # I want the original (120,120) size vertices output
    crop_params = np.array([[1/scale_x, 0, -sx/scale_x],
                            [0, 1/scale_y, -sy/scale_y],
                            [0, 0, 1]])
    # print('manual params: ', crop_params)
    ...
    return vertex, my_vertices, crop_params
