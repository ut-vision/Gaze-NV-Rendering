...
def main(args):
    ...
    for img_fp in img_list:
        ...
        for rect in rects:
            ...
            if args.dump_ply:
                dump_to_ply(vertices, tri, '{}_{}.ply'.format(out_fp.replace(suffix, ''), ind))
            if args.dump_vertex:
                dump_vertex(vertices, '{}_{}.mat'.format(out_fp.replace(suffix, ''), ind))
            if args.dump_pts:
                ...
                crop_pts[2,:] *= -1
                cfp = '{}_lm.txt'.format(crop_fp.replace(suffix, ''))
                np.savetxt(cfp, crop_pts.T, fmt='%.3f')
                print('Save cropped 68 3d landmarks to {}'.format(cfp))




            if args.dump_roi_box:
                wfp = '{}_{}.roibox'.format(out_fp.replace(suffix, ''), ind)
                np.savetxt(wfp, roi_box, fmt='%.3f')
                print('Save roi box to {}'.format(wfp))
            if args.dump_paf:
                wfp_paf = '{}_{}_paf.jpg'.format(out_fp.replace(suffix, ''), ind)
                wfp_crop = '{}_{}_crop.jpg'.format(out_fp.replace(suffix, ''), ind)
            ...
            if args.dump_obj:
                ...
                cfp = '{}.obj'.format(crop_fp.replace(suffix, ''))
                write_obj_with_colors(cfp, crop_vertices, tri, colors)
                print('Dump cropped obj with sampled texture to {}'.format(cfp))
                pfp = '{}_crop_params.txt'.format(crop_fp.replace(suffix, ''))
                np.savetxt(pfp, crop_params, fmt='%.4f')

                # pose_t = np.concatenate((pose, t),axis=0)
                # wfp = out_fp.replace(suffix, '_pose.txt')
                # np.savetxt(wfp, pose, fmt='%.3f')
                # print('save pose to {}'.format(wfp))
        ...
        if args.dump_pose:
            ...
            wfp = out_fp.replace(suffix, '_pose.jpg')
            cv2.imwrite(wfp, img_pose)
            print('Dump to {}'.format(wfp))
            

        if args.dump_depth:
            wfp = out_fp.replace(suffix, '_depth.png')
            ...
            print('Dump to {}'.format(wfp))
        if args.dump_pncc:
            wfp = out_fp.replace(suffix, '_pncc.png')
            pncc_feature = cpncc(img_ori, vertices_lst, tri - 1)  # cython version
            cv2.imwrite(wfp, pncc_feature[:, :, ::-1])  # cv2.imwrite will swap RGB -> BGR
            print('Dump to {}'.format(wfp))
        if args.dump_res:
            draw_landmarks(img_ori, pts_res, wfp=out_fp.replace(suffix, '_3DDFA.jpg'), show_flg=args.show_flg)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='3DDFA inference pipeline')
    parser.add_argument('-f', '--files', type=str, help='image files paths fed into network, single or multiple images')
    parser.add_argument('-output', '--output_path', help='output path')                    
